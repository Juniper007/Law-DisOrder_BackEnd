...
sourceTableSchema.statics.addNewDatasets = async function (data) {
  let dataset = await this.findOne({ sourceName: data.sourceName });
  if (dataset) {
    throw new Error("Dataset already exists!");
  } else {
...
sourceTableSchema.methods.hi = function () {
  console.log('hi')
}

sourceTableSchema.statics.CheckForUpdates = async function (data, date) {
  let dataset = await this.findOne({ sourceName: data.sourceName });
  if (!dataset) {
...
const SourceTable = mongoose.model(
